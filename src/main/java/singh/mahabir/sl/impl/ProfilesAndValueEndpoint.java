...
public class ProfilesAndValueEndpoint {
    ...
    /*
     * We can also set the value from system properties with the same syntax. Let’s assume that we have defined a system property
     * named systemValue
     */
    @Value("${systemValue}")
     ...
     * Suppose we had a property priority defined as a system property with the value “System property” and defined as something
     * else in the properties file. In the following code the value would be “System property”:
     */
    @Value("${priority}")
    private String prioritySystemProperty;

    /*
     * Sometimes we need to inject a bunch of values. It would be convenient to define them as comma-separated values for the
     * single property in the properties file or as a system property and to inject into an array. In the first section, we
     * defined comma-separated values in the listOfValues of the properties file, so in the following example the array values
     * would be [“A”, “B”, “C”]:
     * catch in private String[] valuesArray;
     */
    @Value("${listOfValues}")
    ...
    /*
     * If we have not defined the system property, then the null value will be assigned. To prevent this, we can provide a default
     * value in the SpEL expression. In the following example, we get “some default” value for the field if the system property is
     * not defined:
     */
